...
async function getFeatureList(){
    ...
    try{
        const response = await fetch("https://emgrew24.github.io/gamerdailies.github.io/db.json");
...
async function getAdditionalFeatures(){
     ...
     try{
        ...
        for (let service of data.services){
            ...
            // Ensure that the first 3 features are skipped
            if(service.id >=4 ){
                const feature = document.createElement('div');
...
// Call both functions to insert the json database data onto the website
getFeatureList();
